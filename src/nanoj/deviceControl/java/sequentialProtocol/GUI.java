...
import nanoj.deviceControl.java.sequentialProtocol.tabs.DirectControl;
import nanoj.deviceControl.java.sequentialProtocol.tabs.DeviceCalibration;
import nanoj.deviceControl.java.sequentialProtocol.tabs.DeviceConnections;
import nanoj.deviceControl.java.sequentialProtocol.tabs.SequentialLabelling;
...
public final class GUI {
    ...
    void create(CMMCore core) throws Exception {
        ...
        deviceManager.setCore(core);
        deviceManager.loadPlugins();
        // GUI objects and layout.
        mainFrame = new JFrame("Device Control and Sequential Protocol");
        ...
        //Tab objects
        deviceConnections = new DeviceConnections(this);
        directControl = new DirectControl(this);
        deviceCalibration = new DeviceCalibration(this);
        ...
        topPane.addTab(deviceConnections.name, deviceConnections);
        topPane.addTab(directControl.name, directControl);
        topPane.addTab(deviceCalibration.name, deviceCalibration);
    ...
    public void dispose() {
        ...
        deviceConnections.rememberSettings();
    ...
    public String stopAllDevices() throws Exception {
        if (deviceManager.noDevicesConnected()) return "No device connected.";
        deviceManager.stopAllDevices();
        return "Stopping all devices!";
    }

    public String stopDevice(int device) throws Exception {
        if (deviceManager.noDevicesConnected()) return "No device connected.";
        else if (device > deviceManager.getConnectedDevicesList().size() || device < 1)
            return "Index is not valid. Has to be between 1 and N connected devices.";
        else return "Stopping device " + deviceManager.stopPumping(device-1);
    }
