...
import nanoj.deviceControl.java.devices.*;
...
public class Step extends Observable implements Observer, ActionListener {
    ...
    */
    ...
    private JComboBox action = new JComboBox(Device.Action.values());
    private int number;
    ...
    public Step(int num) {
        this(num,"Name",true,false,1,TimeUnit.SECS, Syringe.PERISTALTIC,500,VolumeUnit.ML, Device.Action.Infuse);
    }

    Step(int num, String name, boolean suck, boolean exchange, int time, TimeUnit timeUnit,
         Syringe syringe, double volume, VolumeUnit volumeUnit, Device.Action action)
    {
        ...
        if (deviceManager.noDevicesConnected())
            devices = new String[]{DeviceManager.NO_PUMP_CONNECTED};
        else
            devices = deviceManager.getAllFullNames();
        ...
        deviceList.setPrototypeDisplayValue(DeviceManager.NO_PUMP_CONNECTED);
        ...
        if (arg.equals(DeviceManager.NEW_PUMP_CONNECTED) ||
            ...
            if (deviceManager.noDevicesConnected()) {
                ...
                for (ConnectedSubDevice device: deviceManager.getConnectedDevicesList())
                    deviceList.addItem(device.getFullName());
            ...
            rateSlider.setDeviceSelection(deviceList.getSelectedIndex());
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource().equals(deviceList)) {
    ...
    class Listener implements ActionListener {
        ...
        void updateSyringeInformation() {
            ...
            if (deviceManager.anyDevicesConnected()) {
                rateSlider.setDeviceSelection(deviceList.getSelectedIndex());
    ...
    public HashMap<String,String> getStepInformation() {
        ...
        info.put("device", (String) deviceList.getSelectedItem());
    ...
    public void addActionListener(ActionListener a) {
        deviceList.addActionListener(a);
    }

    public void updateStepInformation(HashMap<String,String> givenInformation) {
        String device = givenInformation.get("device");

        for (int i = 0; i < deviceList.getItemCount(); i++) {
            if (device.equals((String) deviceList.getItemAt(i))) {
                if (deviceManager.isConnected(i))
                    deviceList.setSelectedItem(device);
                break;
            }
        }
    ...
    public Device.Action getAction() {
        return Device.Action.valueOf(action.getSelectedItem().toString());
    }
